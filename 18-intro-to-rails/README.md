...
* Create views
* Generate a migration to add a column to a table
* Generate a model
...
[Built With Rails](https://www.netguru.co/blog/top-10-sites-built-with-ruby-on-rails)
...
Rails has a generator called `scaffold` that will create the whole MVC structure for a resource, let's say that inside the grocery_store app, we want the `Employee` resource to have a name, title, and salary, we would type:

```ruby
rails g scaffold Employee name title salary:integer
```
...
Take a look at the controller, it has all the RESTful methods, and these methods already contain the code to query the database through the model `Employee`.
